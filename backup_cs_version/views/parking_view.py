...
class ParkingAnalysisView(ctk.CTkFrame):
    ...
    def create_query_form(self, parent):
        ...
        ctk.CTkLabel(form_frame, text="停车场选择:", font=ctk.CTkFont(size=14, weight="bold")).pack(anchor="w", pady=(5, 8))
        self.parking_lot_var = ctk.StringVar(value=self.controller.get_parking_lots()[0])
        parking_frame = ctk.CTkFrame(form_frame, fg_color="transparent")
        parking_frame.pack(fill="x", pady=(0, 15))
        for lot in self.controller.get_parking_lots():
            ctk.CTkRadioButton(
                parking_frame,
                text=lot,
    ...
    def query_data(self):
        parking_lot = self.parking_lot_var.get()
        chart_type = self.chart_type_var.get()
        start_date = self.start_date.get()
        end_date = self.end_date.get()
        
        result = self.controller.analyze_parking_data(
